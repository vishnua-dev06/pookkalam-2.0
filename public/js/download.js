function getExportFilename(filename) {
  // Prefixes exported filenames with the project name, preserving extension.
  return `poovum-codeum-pookalam-${filename}`;
}
...
function addLogoToCanvas(ctx, width, height) {
  ...
  logo.onload = function() {
    ...
    ctx.fillText('POOVUM CODEUM', 80, height - 30); // Adjusted position
  };
...
function addLogoToSVG(svg, width, height) {
  ...
  text.textContent = 'POOVUM CODEUM';
...
function downloadPDF(canvasId, filename) {
  ...
  logo.onload = function() {
    ...
    tempCtx.fillText('POOVUM CODEUM', 80, tempCanvas.height - 30); // Adjusted position
